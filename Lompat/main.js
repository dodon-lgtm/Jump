...
const bgMusic = new Audio('image/normal.mp3');
bgMusic.loop = true;      // ulangi terus
...
(() => {
    ...
    function loop(now) {
        ...
        if (running) {
            ...
            // Collision check
            // Collision check (hitbox diperkecil)
            const shrinkP = 0.2; // 20% lebih kecil dari sisi player
            const pBox = {
                x: player.x + player.w * shrinkP,
                y: player.y + player.h * shrinkP,
                w: player.w * (1 - shrinkP * 2),
                h: player.h * (1 - shrinkP * 2),
            };
            for (const o of obstacles) {
                const shrinkO = 0.25; // 25% lebih kecil dari sisi obstacle
                const oBox = {
                    x: o.x + o.w * shrinkO,
                    y: o.y + o.h * shrinkO,
                    w: o.w * (1 - shrinkO * 2),
